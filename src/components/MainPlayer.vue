  ...
  <v-container>
    <v-row justify="center">
      <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        ...
        <v-card style="background-color:#2d2d2d;">
          <v-toolbar dark color="black">
            <v-btn icon dark @click="miniplayer">
              <v-icon>mdi-close</v-icon>
            </v-btn>
            <v-btn icon dark color="#FF8C00">
              <v-icon>mdi-music-box-outline</v-icon>
            </v-btn>
            <v-row style="width:200px;padding-left: 15px;">
              <h6 style="color:#FF8C00;font-size: 30;">Coming Next</h6>
            </v-row>
            ...
            <div class="player">
              <div class="player__top">
                ...
                <div class="player-controls">
                  ...
                  <div target="_blank" class="player-controls__item" @click="shareSong">
                    <svg class="icon">
              ...
              <div class="progress" ref="progress">
                <div class="progress__top">
                  <div class="album-info" v-if="currentTrack">
                    <div class="album-info__name">{{ vidData.title }}</div>
                    <div class="album-info__track">{{ vidData.channelTitle }}</div>
                  </div>
        ...
        <div class="text-center">
          <v-bottom-sheet v-model="sheetShare">
            ...
            <v-list>
              <v-subheader style="font-size:20px;font-weight:bold;color:black">Share Song</v-subheader>
              <v-list-item @click="this.sheetShare = false">
                <v-list-item-avatar>
                  <v-avatar>
                ...
                <v-list-item-title>
                  <ShareNetwork
                    ...
                    quote
                    style="color:#3b5998"
                    hashtags="SyncMusic,vaibhavrajput"
              ...
              <v-list-item @click="this.sheetShare = false">
                <v-list-item-avatar>
                  <v-avatar>
                    <v-icon style="color:#4AC959">mdi-whatsapp</v-icon>
                  </v-avatar>
                </v-list-item-avatar>
                <v-list-item-title>
                  <ShareNetwork
                    ...
                    style="color:#4AC959"
                    hashtags="SyncMusic,vaibhavrajput"
                  >Share on Whatsapp</ShareNetwork>
                </v-list-item-title>
              </v-list-item>
              ...
              <v-list-item @click="this.sheetShare = false">
                <v-list-item-avatar>
                  <v-avatar>
                    <v-icon style="color:#0088cc">mdi-telegram</v-icon>
                  </v-avatar>
                </v-list-item-avatar>
                <v-list-item-title>
                  <ShareNetwork
                    ...
                    style="color:#0088cc"
                    hashtags="SyncMusic,vaibhavrajput"
                  >Share on Telegram</ShareNetwork>
                </v-list-item-title>
              </v-list-item>
              ...
              <v-list-item @click="this.sheetShare = false">
                <v-list-item-avatar>
                  <v-avatar>
                    <v-icon style="color:#00acee">mdi-twitter</v-icon>
                  </v-avatar>
                </v-list-item-avatar>
                <v-list-item-title>
                  <ShareNetwork
              ...
              <v-list-item @click="copyurl">
                <v-list-item-avatar>
                  <v-avatar>
                    <v-icon style="color:gray">mdi-content-copy</v-icon>
                  </v-avatar>
                </v-list-item-avatar>
                <v-list-item-title style="color:gray">Copy Song Link</v-list-item-title>
              </v-list-item>
            </v-list>
          </v-bottom-sheet>
        </div>
        ...
        <div class="text-center">
          ...
          <div class="text-center">
            <!-- <v-btn color="deep-purple accent-4" class="white--text" @click="temp">
              Launch Application
              <v-icon right>mdi-open-in-new</v-icon>
            </v-btn>-->

            <v-overlay :value="overlay">
              <v-progress-circular color="#ff8c00" indeterminate size="64"></v-progress-circular>
            </v-overlay>
          </div>
        </div>
      </v-dialog>
...
export default {
  data() {
    return {
      ...
      Playlist: [],
      songindex: 0,
      musickey: "d7beb290acmsh24f07eab39705c5p106c07jsn9e53ecbf2183"
    };
  },
  methods: {
    copyurl() {
      copy(this.currentTrack);
      this.sheetShare = false;
      this.snackbar = true;
    },
    shareSong() {
    ...
    Download() {
      axios
        .get("https://tinyurl.com/y9oxvo26")
        .then(responce => {
    ...
    getSongUrl(id) {
      if (this.apiCount == 0) {
        axios
          .get(
            "https://youtube-to-mp32.p.rapidapi.com/yt_to_mp3?video_id=" + id,

            {
              headers: {
                "x-rapidapi-host": "youtube-to-mp32.p.rapidapi.com",
                "x-rapidapi-key": this.musickey
              }
            }
          )
          .then(response => {
            //5th key d7beb290acmsh24f07eab39705c5p106c07jsn9e53ecbf2183 vaibhavAlonesniper
            // 4th key e69bd46ceemshb885ff26111d706p18dc05jsn2f0e081751ce rawmusic
            //3rd key cf6610e0c8msh76908cf5b5f43dep18efdejsn1f1ded24ae7e vaibhav.p
            //2nd key a107aba38dmshbc3ba3d355d1208p17956djsn40b36af6e033 office
            //1st ket 62c24e4dfamsh084f9b282ea476cp1592b6jsn66e58040773c vaibhavtest
            console.log(response.status);
            // Send the event on a channel (i-got-clicked) with a payload (the click count.)
            this.overlay = true;
            this.currentTrack = response.data.Download_url;
          ...
          .catch(error => {
            if (this.apiCount == 0) {
              this.apiCount = 1;
              this.musickey =
                "e69bd46ceemshb885ff26111d706p18dc05jsn2f0e081751ce";
              this.getSongUrl(id);
              console.log(error);
            }
            if (this.apiCount == 1) {
              this.apiCount = 2;
              this.musickey =
                "cf6610e0c8msh76908cf5b5f43dep18efdejsn1f1ded24ae7e";
              this.getSongUrl(id);
              console.log(error);
            }
            if (this.apiCount == 2) {
              this.apiCount = 3;
              this.musickey =
                "a107aba38dmshbc3ba3d355d1208p17956djsn40b36af6e033";
              this.getSongUrl(id);
              console.log(error);
            }
            if (this.apiCount == 3) {
              this.apiCount = 0;
              this.musickey =
                "62c24e4dfamsh084f9b282ea476cp1592b6jsn66e58040773c";
              this.getSongUrl(id);
              console.log(error);
            }
          });
    ...
    miniplayer() {
      this.dialog = false;
      EventBus.$emit("miniplayer", this.vidData);
    },
    ...
    prevsong() {
      console.log(this.songindex + "   pre");
      if (this.songindex > 1) {
        this.audio.pause();
        console.log(this.songindex + "   post");
        this.vidData = this.Playlist[this.songindex - 2].snippet;
        this.audio = null;
        //console.log(this.Playlist[0].id.videoId);
        this.cover = this.Playlist[
          this.songindex - 2
        ].snippet.thumbnails.high.url;
        this.getSongUrl(this.Playlist[this.songindex - 2].id.videoId);
        //   this.audio.src = this.currentTrack.source;
        this.songindex = this.songindex - 1;
        ///Database code
        db.song_history
          .where({ vidid: this.Playlist[0].id.videoId })
          .first(song => {
        ...
        var insert_object = {
          vidid: this.Playlist[0].id.videoId,
          urlthumb: this.Playlist[this.songindex].snippet.thumbnails.default
            .url,
          title: this.vidData.title,
          channelname: this.vidData.channelTitle,
          islike: 0
        };
        ...
        alert("Previouse song will work after played 2 songs from Playlist");
        // console.log("else");
        // axios
        //   .get(
        //     "https://www.googleapis.com/youtube/v3/search?part=snippet&relatedToVideoId=" +
        //       this.Playlist[3].id.videoId +
        ...
        //     console.log(erroe);
        //   });
        // this.vidData = this.Playlist[0].snippet;
        // this.cover = this.Playlist[0].snippet.thumbnails.high.url;
        // this.getSongUrl(this.Playlist[0].id.videoId);
        // ///Database code
        // db.song_history
        //   .where({ vidid: this.Playlist[0].id.videoId })
        //   .first(song => {
        ...
        //   islike: 0
        // };
        // db.song_history.add(insert_object1).catch(error => {
        //   console.log(error);
        // });
        // this.CheckDb();
        // //Databse code end
    ...
    resetPlayer() {
      if (this.songindex < 4) {
        this.audio.pause();
        this.vidData = this.Playlist[this.songindex].snippet;
        // this.barWidth = 0;
        // this.circleLeft = 0;
        // this.audio.currentTime = 0;
        this.audio = null;
        console.log(this.Playlist[0].id.videoId);
        this.cover = this.Playlist[this.songindex].snippet.thumbnails.high.url;
        this.getSongUrl(this.Playlist[this.songindex].id.videoId);
        //   this.audio.src = this.currentTrack.source;
        this.songindex = this.songindex + 1;
    ...
    favorite() {
      ...
      db.song_history
        ...
        .first(song => {
          db.song_history
            .update(song.id, { islike: 1 })
            .then(function(updated) {
              if (updated) {
                console.log("song liked");
              } else console.log("error:islike not updated");
            });
        })
        .catch(error => {
          console.log(error);
  ...
  mounted() {
    EventBus.$on("PlaySongData", data => {
      this.getSongUrl(data.id);
      this.vidID = data.id;
      this.vidData = data.vidData;

      this.cover = data.vidData.thumbnails.high.url;
      this.dialog = true;
      axios
        .get(
          "https://www.googleapis.com/youtube/v3/search?part=snippet&relatedToVideoId=" +
    ...
    EventBus.$on("PlaySongDataHistory", data => {
      this.getSongUrl(data.id);
      this.vidID = data.id;
      this.vidData = data.snipetData;
      this.cover = data.snipetData.thumbnails.high.url;
      this.dialog = true;
      this.audio.pause();
      console.log(data);
    });
    ...
    EventBus.$on("maximized", data => {
      this.dialog = data;
    });
    EventBus.$on("PlayPause", data => {
      this.play();
      console.log(data);
    });
    EventBus.$on("StopSong", data => {
      if (this.audio != null) {
        this.audio.pause();
        this.Playlist = null;
        this.songindex = 0;
